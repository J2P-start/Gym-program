...
- **Log the variable, not the constant.** Where the prescription fixes the clock (a 10-minute erg piece), track distance — recording `10:00` against a prescription of `10 min` carries no information. Where it fixes the distance (a 300 m run leg, a 50 m sled), track time. Second-based holds are the exception: for a dead hang the time *is* the variable. Tests in `data/hyrox.test.js` enforce both directions.
- Station stats key off **exercise name**, so a non-station exercise must never reuse a station's name — Base Friday's heavy grip carry is `Heavy farmers carry`, not `Farmers carry`, or it would be averaged into the race-spec station rate. A test guards this.
...
- **Sunday is always easy and recovery-first.** Quality running lives on Wednesday; a threshold session on the recovery day (the day after Saturday BJJ) was the hardest running of the week landing on the softest day. `base-run-threshold` stays defined but unscheduled so old logs resolve. Base and Build run 7 training days with no full rest day, so Sunday's copy makes skipping the run for sauna-only an explicit judgement call rather than a failure.
